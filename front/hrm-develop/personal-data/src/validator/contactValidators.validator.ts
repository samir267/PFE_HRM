...
export const phoneValidationSchema = Joi.object({
  phoneType: Joi.number().valid(...Object.values(PhoneType)).required(),
  isPrimary: Joi.boolean().default(false),
  countryCode: Joi.string().max(5).required(),
  phoneNumber: Joi.string().max(20).min(8).required(),
...
export const electronicValidationSchema = Joi.object({
  contactType: Joi.string().valid(...Object.values(ElectronicContactType)).required(),
  isPrimary: Joi.boolean().default(false),
