...
const HireCandidate: React.FC = () => {
  ...
  useEffect(() => {
    const fetchApplicants = async () => {
      try {
        setIsLoading(true);
        const response = await fetch('http://localhost:4000/api/v1/kafka/getemployees');
        if (!response.ok) {
        ...
        const mappedApplicants: Applicant[] = data.map((employee: any) => ({
          id: employee._id,
          timestamp: employee.createdAt || new Date().toISOString().slice(0, 19).replace('T', ' '),
          name: `${employee.firstName} ${employee.lastName}`.trim(),
          applicantId: employee.registrationNumber || `EMP${Math.floor(Math.random() * 10000)}`,
          status: mapHiringStage(employee), // Map API status to one of the allowed stages
          department: employee.department || '',
          position: employee.jobTitle || '',
          email: employee.workEmail || employee.personalEmail || '',
          phone: employee.mobilePhone || '',
          address: employee.homeAddress
            ? `${employee.homeAddress.street1}, ${employee.homeAddress.city}, ${employee.homeAddress.country}`
            : '',
        }));
        setAllApplicantList(mappedApplicants);
        setFilteredApplicantList(mappedApplicants);
        // Set default selected applicant (first one, if available)
        ...
        addNotification('Failed to load applicants from API.', 'error');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  // Helper function to map API data to hiring stage
  const mapHiringStage = (employee: any): Applicant['status'] => {
    // Example mapping; adjust based on actual API data
    // Assuming the API doesn't provide a direct hiring stage, we can infer or use a default
    return 'Pending Review'; // Default status; customize based on your API data
  };

  // Effect to update applicant details when selected applicant changes
  useEffect(() => {
    if (selectedApplicant && allApplicantList.length > 0) {
      const employee = allApplicantList.find((app) => app.id === selectedApplicant.id);
      if (employee) {
        // Fetch individual employee details if needed, or map from API data
        const fetchEmployeeDetails = async () => {
          try {
            const response = await fetch(`http://localhost:4000/api/v1/kafka/employee/${selectedApplicant.id}`);
            if (!response.ok) {
              throw new Error(`HTTP error! Status: ${response.status}`);
            }
            ...
            setApplicantDetails({
              ...
              workIdentifier: employeeData.corporateId || selectedApplicant.applicantId,
              origin: employeeData.origin || 'Company Website',
              externalUserId: employeeData.nationalNumber || '',
              servesAsId: `HR-${employeeData.corporateId || selectedApplicant.applicantId}`,
              timestampApplication: employeeData.createdAt || selectedApplicant.timestamp,
              reason: '',
              hiringStage: selectedApplicant.status,
              company: 'Fluxeruim BANK', // Static or map from API if available
              expectedSalary: employeeData.expectedSalary || '',
              notes: employeeData.notes || 'No specific notes yet.',
              dob: employeeData.birthDate || '',
              nationality: employeeData.citizenships?.[0] || '',
              linkedInProfile: employeeData.linkedInProfile || '',
            });
          } catch (err) {
            console.error('Error fetching employee details:', err);
            addNotification('Failed to load applicant details.', 'error');
          }
        };
  ...
  const handleSaveApplicantDetails = () => {
    if (!selectedApplicant || !applicantDetails) return;
      ...
      prevList.map((applicant) =>
        applicant.id === selectedApplicant.id
          ? {
              ...
              status: applicantDetails.hiringStage,
  ...
  const openConfirmationModal = (actionType: string) => {
    ...
    switch (actionType) {
      case 'hire':
        content = `Are you sure you want to hire ${selectedApplicant?.name || 'the applicant'}? This action will update their status to "Hired".`;
        action = handleHireApplicant;
        break;
      case 'reject':
        content = `Are you sure you want to reject ${selectedApplicant?.name || 'the applicant'}? This action will update their status to "Rejected".`;
  ...
  const handleHireApplicant = () => {
    if (!selectedApplicant || !applicantDetails) return;
    setApplicantDetails({ ...applicantDetails, hiringStage: 'Hired' });
    setAllApplicantList((prevList) =>
  ...
  const handleRejectApplicant = () => {
    if (!selectedApplicant || !applicantDetails) return;
    setApplicantDetails({ ...applicantDetails, hiringStage: 'Rejected' });
    setAllApplicantList((prevList) =>
  ...
  const handleAddNewApplicant = () => {
    ...
    setAllApplicantList((prevList) => [newApplicant, ...prevList]);
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
            ...
            <div className="space-y-4 text-sm text-gray-700">
              <div className="flex items-start">
                ...
                <div>
                  <p>
                    <span className="font-semibold">{applicantDetails.timestampApplication.split(' ')[0]}:</span> Status changed to "{selectedApplicant.status}".
                  </p>
                  <p className="text-gray-500 text-xs">By System</p>
                </div>
              </div>
              <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-2xl font-semibold text-gray-900">
                {allApplicantList.filter((a) => a.status === 'Hired' && a.timestamp.startsWith('2025-08')).length}
              </p>
            </div>
          </div>
          <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 flex items-center space-x-4">
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
            ...
            <p className="text-gray-700 mb-2">
              Total Recruitment Spend (August): <span className="font-semibold text-orange-700">€ 12,000</span>
            </p>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value="Expert" className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Email</label>
    ...
    <div className="min-h-screen bg-gray-100 font-sans">
      ...
      <nav className="bg-orange-600 p-4 text-white shadow-md flex justify-between items-center">
        <div className="flex items-center space-x-4">
          ...
          <span className="ml-6 text-sm text-orange-200">Welcome, Expert</span>
        ...
        <div className="flex flex-col lg:flex-row p-6 space-y-6 lg:space-y-0 lg:space-x-6 h-[calc(100vh-80px)]">
          ...
          <div className="bg-white p-6 rounded-lg shadow-lg border border-gray-200 lg:w-1/3 flex flex-col min-h-full max-h-full overflow-hidden">
            ...
            <div className="overflow-y-auto flex-1 pr-2 -mr-2">
              {isLoading ? (
                <div className="text-center text-gray-500 py-10">Loading applicants...</div>
              ) : error ? (
